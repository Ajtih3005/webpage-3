...
async function getSubscriptionPage(slug: string) {
  ...
  const subscriptions = pageSubscriptions?.map((ps) => ps.subscriptions).filter(Boolean) || []
...
export default async function SubscriptionPageDisplay({ params }: PageProps) {
  ...
  const handleSelectPlan = (subscriptionId: number) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          {page.subtitle && <p className="text-2xl text-amber-700 mb-6">{page.subtitle}</p>}
        </div>
