...
interface Subscription {
  ...
  auto_activate_after_days?: number
...
export default function EditSubscription({ params }: { params: { id: string } }) {
  ...
  async function fetchSubscription() {
    try {
      ...
      if (data) {
        ...
        setAutoActivateAfterDays(data.auto_activate_after_days?.toString() || "0")
        ...
        if (data.features && Array.isArray(data.features)) {
          // Separate custom features from predefined ones
          const customFeatures: string[] = []
          const predefinedFeatures: string[] = []

          data.features.forEach((feature: string) => {
            if (PREDEFINED_FEATURES.includes(feature)) {
              predefinedFeatures.push(feature)
            } else {
          ...
          setSelectedPredefinedFeatures(predefinedFeatures)
        }

        // Determine if using dates
        setUseDates(data.start_date !== null || data.end_date !== null)

        // Set start and end dates if they exist
        if (data.start_date) {
          setStartDateStr(new Date(data.start_date).toISOString().split("T")[0])
        }
        if (data.end_date) {
          setEndDateStr(new Date(data.end_date).toISOString().split("T")[0])
          // If both dates exist, check if we should use custom end date
          if (data.start_date) {
            const startDate = new Date(data.start_date)
            const endDate = new Date(data.end_date)
            const calculatedDays = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24))

            // If the calculated days don't match the stored duration_days, use custom end date
            if (calculatedDays !== data.duration_days) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If activation date is set, update all pending subscriptions
      if (requiresActivation && activationDate) {
        try {
          // Update all pending subscriptions for this subscription plan
          const { error: updateError } = await supabase
            .from("user_subscriptions")
            .update({
              activation_date: activationDate.toISOString(),
              is_active: false, // Keep them inactive until the activation date
            })
            .eq("subscription_id", subscriptionId)
            .eq("is_active", false)

          if (updateError) {
            console.error("Error updating pending subscriptions:", updateError)
          }
        } catch (err) {
          console.error("Error updating pending subscriptions:", err)
        }
      }
