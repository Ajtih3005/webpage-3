...
export async function GET() {
  try {
    ...
    if (count && count >= 43) {
      // Make sure all existing reviews are published
      const { error: updateError } = await supabaseAdmin.from("reviews").update({ is_published: true }).lt("id", 44) // Ensure we only update the first 43 reviews
      ...
      return NextResponse.json({
        success: true,
        message: `Already have ${count} reviews in the database. All reviews are now published.`,
        count,
      })
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully added ${reviewsToInsert.length} new reviews. Total: ${newCount}. All reviews are now published.`,
